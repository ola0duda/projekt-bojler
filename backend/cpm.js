...
function buildCriticalPath(tasks, tasksById) {
  // Tworzenie ścieżki krytycznej LC
  ...
  // Start ścieżki LC
  const startTasks = criticalTasks.filter((task) => task.dependencies.length === 0);
  ...
  function traverse(taskId) {
    ...
    // Szuaknie kolejnych zadań w ścieżce LC
    const criticalSuccessors = task.successors.filter((successorId) => {
      const successor = tasksById.get(successorId);
      return successor && successor.isCritical && task.ef === successor.es;
    });

    if (criticalSuccessors.length > 0) {
      traverse(criticalSuccessors[0]);
    }
  }

  traverse(startTasks[0].id);
...
function analyzeProject(rawTasks) {
  ...
  const criticalPath = buildCriticalPath(tasks, tasksById);
